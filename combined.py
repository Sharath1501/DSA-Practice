...
def run_deepsort_on_video_original(video_path):
    ...
    plt.title('Original Deep SORT Frame Processing Time Distribution')

    plt.subplot(2, 1, 2)
    plt.plot(np.cumsum(frame_times))
    plt.xlabel('Frame Index')
    plt.ylabel('Cumulative Time (seconds)')
    plt.title('Original Deep SORT Cumulative Processing Time')

    plt.tight_layout()

    # Save the plot as an image in the current directory
    plt.savefig('original_deep_sort_performance.png')
...
def run_deepsort_on_video_optimized(video_path, skip_interval=5):
    ...
    plt.xlabel('Frame Index')
    plt.ylabel('Frame Processing Time (seconds)')
    plt.title('Optimized Deep SORT Frame Processing Time Distribution')

    plt.subplot(2, 1, 2)
    plt.plot(np.cumsum(frame_times))
    plt.xlabel('Frame Index')
    plt.ylabel('Cumulative Time (seconds)')
    plt.title('Optimized Deep SORT Cumulative Processing Time')

    plt.tight_layout()

    # Save the plot as an image in the current directory
    plt.savefig('optimized_deep_sort_performance.png')

    plt.show()

if __name__ == "__main__":
    video_path = 'l.mp4'
    run_deepsort_on_video_original(video_path)
    run_deepsort_on_video_optimized(video_path, skip_interval=5)
